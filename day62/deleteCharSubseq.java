...
class Solution{
    public static void main(String[] args){
        ...
        int c=0;
        boolean[]deleted=new boolean[orig.length()];
        for(int i=0;i<n;i++){
            deleted[indices[i]]=true;
            if(!isSubSeq(sub,orig,deleted)){
                break;
            }
            c++;
        }
    ...
    public static boolean isSubSeq(String sub, String orig,boolean[]deleted){
        int i=0,j=0;
        while(i<sub.length()  && j< orig.length()){
            if(!deleted[j] && sub.charAt(i)==orig.charAt(j)){
                i++;
            }
            j++;
        }
