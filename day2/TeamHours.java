...
class TeamHours{
    public static void main(String[] args){
        ...
        int points=0;
        points+=cal(sumTask,sumHours,lower,upper,workGoal);
        ...
        for(int i=k;i<n;i++){
            ...
            sumHours-=hours[i-k];
            sumHours+=hours[i];
            //System.out.println(sumTask+" "+sumHours);
            points+=cal(sumTask,sumHours,lower,upper,workGoal);
            //System.out.println(points);
        }
        System.out.println(points);
    }
    public static int cal(int T,int E,int lower,int upper,int workGoal){
        if(T<lower && E>=workGoal) return -2;
        if(T<lower && E<workGoal)return -1;
        if(T>=upper && E>=workGoal)return 1;
        if(T>=upper && E<workGoal)return 2;
        return 0;
