...
//here we r given infinite links..
LC=>Meeting room 3, we had limited number of keys..
*/
import java.util.*;
class meetingRoom{
    public static void main(String[] args) {
        ...
        Arrays.sort(arr,(a,b)->a[0]-b[0]);//asend
        int nextlink=0;
        PriorityQueue<Integer>freeLink=new PriorityQueue<>();
        PriorityQueue<int[]>busy=new PriorityQueue<>((a,b)->a[0]-b[0]);//{end, link} in busy state
        for(int a[]:arr){
            int start=a[0];
            int end=a[1];
            int idx=a[2];
            //remove the meetingsthat are completed..
            while(!busy.isEmpty() && busy.peek()[0]<=start){
                freeLink.offer(busy.poll()[1]);//add that link to free queue..
            }
            int assignedLink;
            if(!freeLink.isEmpty()){
                assignedLink=freeLink.poll();
            }
            else{
                assignedLink=nextlink++;
            }
            busy.offer(new int[]{end,assignedLink});
            if(idx==c){
                System.out.println(assignedLink);
            }
        }
