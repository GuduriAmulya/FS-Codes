...
17*2=>34 (4A,6B) 2A more we need.. +3*2=>34+6=>40
Sample Input-2:
---------------
3 2
1 2 3
...
class ShoppingOffers{
    ...
    public static void main(String[] args){
        ...
        int n=sc.nextInt();//total items..
        int k=sc.nextInt();//offers..
        int price[]=new int[n];
        for(int i=0;i<n;i++){
            price[i]=sc.nextInt();
        }
        ...
        int ans=dfs(price, offers,req);
        System.out.println(ans);
    }
    public static int dfs(int[]price, int[][]offers, int[]req){
        String key=Arrays.toString(req);
        if(mp.containsKey(key))return mp.get(key);
        //buy individually..
        int minCost=0;
        for(int i=0;i<req.length;i++){
            minCost+=(req[i]*price[i]);
        }
        //try every offer
        //iterate over all items for every offer...
        for(int[]o:offers){
            boolean valid=true;
            ...
            if(valid){
                minCost=Math.min(minCost,o[req.length]+dfs(price,offers,newReq));
            }
        }
        mp.put(key,minCost);
        return minCost;
