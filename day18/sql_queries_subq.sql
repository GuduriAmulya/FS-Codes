...
use fs;
select * from emp where
job in (select job from emp where ename='ALLEN');
...
where e.sal>m.sal;
-- try using subqueryy
select e.* from emp e where e.sal>(select sal from emp p where p.empno=e.mgr)
...
use fs;
select e.* from emp e
where e.sal>(select avg(p.sal) from emp p where e.deptno=p.deptno)
