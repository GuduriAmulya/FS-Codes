...
one by one. However, the deactivation process is tricky — when you shut down 
the i-th unit, it triggers a chain reaction with its neighboring units, generating:
...
class Solution{
    ...
    public static int solve(List<Integer>l,int cursum){
        if(l.size()==0){
            max=Math.max(max,cursum);
        }
        for(int i=0;i<l.size();i++){
            int left=(i==0)?1:l.get(i-1);
            int right=(i==l.size()-1)?1:l.get(i+1);
            int gain=left*l.get(i)*right;
            int removed=l.get(i);
            l.remove(i);
            solve(l,cursum+gain);
            l.add(i,removed);
