...
printjson(
    db.deliveries.find({state:"DELIVERED",carrier:"dhl",cost:{$gt:500}},{customerId:1,state:1,cost:1,_id:0})
)
...
db.deliveries.find({
  $or:[{carrier:{$in:["fedex","dh1"]}},{state:{$in:["returned","cancelled"]}}]
})
/*
...
printjson(
    db.deliveries.find({$or:[{carrier: {$in:["fedex","dhl"]}},{state: {$in:["RETURNED","CANCELED"]}}]},{customerId:1,state:1,carrier:1,cost:1,_id:0})
)
...
printjson(
    db.deliveries.find({"packages.sku":"A1","packages.weightKg":{$gte:2}},{customerId:1,"packages.sku":1,"packages.weightKg":1,_id:0})
)
...
printjson(
    db.deliveries.find({$expr:{$gt:[{$size:"$packages"},3]}},
