...
class PrimeSeq{
    public static void main(String[] args){
        ...
        for(int i=0;i<n-k+1;i++){
            String sub=s[0].substring(i,i+k);
            // int num=String.valueOf(sub);
            if(isPrime(sub) && isDivisible(sub,s[0])){
    ...
    public static boolean isDivisible(String sub, String s){
        int v=Integer.parseInt(sub);
        int num=Integer.parseInt(s);
        if(num==0 || v==0)return false;
