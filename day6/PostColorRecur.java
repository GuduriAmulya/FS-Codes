...
class PostColorRecur{
    public static void main(String[] args){
        ...
        for(int i=0;i<3;i++){//for 1st post we have 3 choices..
            int ans=arr[0][i]+solve(1,n,i,arr);
            //System.out.println(ans);
            finalans=Math.min(finalans,ans);
        }
        System.out.println(finalans);
    }
    public static int solve(int index,int n,int prevC,int[][]arr){
        if(index>=n)return 0;
        //2 choices..
        ...
        for(int i=0;i<3;i++){
            if(i!=prevC){
                int a=arr[index][i]+solve(index+1,n,i,arr);
                sum=Math.min(sum,a);
            }
        }
